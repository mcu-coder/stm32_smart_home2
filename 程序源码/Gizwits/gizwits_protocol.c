...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagTemp)
    {
        info->event[info->num] = EVENT_Temp;
        info->num++;
        dataPoints->valueTemp = gizX2Y(Temp_RATIO,  Temp_ADDITION, issuedData->attrVals.valueTemp); 
    }
        
    if(0x01 == issuedData->attrFlags.flagHum)
    {
        info->event[info->num] = EVENT_Hum;
        info->num++;
        dataPoints->valueHum = gizX2Y(Hum_RATIO,  Hum_ADDITION, issuedData->attrVals.valueHum); 
    }
        
    if(0x01 == issuedData->attrFlags.flagPre)
    {
        info->event[info->num] = EVENT_Pre;
        info->num++;
        dataPoints->valuePre = gizX2Y(Pre_RATIO,  Pre_ADDITION, exchangeBytes(issuedData->attrVals.valuePre));
    }
        
    if(0x01 == issuedData->attrFlags.flagGZ_Value)
    {
        info->event[info->num] = EVENT_GZ_Value;
        info->num++;
        dataPoints->valueGZ_Value = gizX2Y(GZ_Value_RATIO,  GZ_Value_ADDITION, exchangeBytes(issuedData->attrVals.valueGZ_Value));
    }
        
    if(0x01 == issuedData->attrFlags.flagMQ2_Value)
    {
        info->event[info->num] = EVENT_MQ2_Value;
        info->num++;
        dataPoints->valueMQ2_Value = gizX2Y(MQ2_Value_RATIO,  MQ2_Value_ADDITION, exchangeBytes(issuedData->attrVals.valueMQ2_Value));
    }
        
    if(0x01 == issuedData->attrFlags.flagMQ7_Value)
    {
        info->event[info->num] = EVENT_MQ7_Value;
        info->num++;
        dataPoints->valueMQ7_Value = gizX2Y(MQ7_Value_RATIO,  MQ7_Value_ADDITION, exchangeBytes(issuedData->attrVals.valueMQ7_Value));
    }
        
    if(0x01 == issuedData->attrFlags.flagMQ135_Value)
    {
        info->event[info->num] = EVENT_MQ135_Value;
        info->num++;
        dataPoints->valueMQ135_Value = gizX2Y(MQ135_Value_RATIO,  MQ135_Value_ADDITION, exchangeBytes(issuedData->attrVals.valueMQ135_Value));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueHum != cur->valueHum)
    {
        GIZWITS_LOG("valueHum Changed\n");
        ret = 1;
    }
    if(last->valuePre != cur->valuePre)
    {
        GIZWITS_LOG("valuePre Changed\n");
        ret = 1;
    }
    if(last->valueGZ_Value != cur->valueGZ_Value)
    {
        GIZWITS_LOG("valueGZ_Value Changed\n");
        ret = 1;
    }
    if(last->valueMQ2_Value != cur->valueMQ2_Value)
    {
        GIZWITS_LOG("valueMQ2_Value Changed\n");
        ret = 1;
    }
    if(last->valueMQ7_Value != cur->valueMQ7_Value)
    {
        GIZWITS_LOG("valueMQ7_Value Changed\n");
        ret = 1;
    }
    if(last->valueMQ135_Value != cur->valueMQ135_Value)
    {
        GIZWITS_LOG("valueMQ135_Value Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueHum = gizY2X(Hum_RATIO,  Hum_ADDITION, dataPoints->valueHum); 

    devStatusPtr->valuePre = exchangeBytes(gizY2X(Pre_RATIO,  Pre_ADDITION, dataPoints->valuePre)); 
    devStatusPtr->valueGZ_Value = exchangeBytes(gizY2X(GZ_Value_RATIO,  GZ_Value_ADDITION, dataPoints->valueGZ_Value)); 
    devStatusPtr->valueMQ2_Value = exchangeBytes(gizY2X(MQ2_Value_RATIO,  MQ2_Value_ADDITION, dataPoints->valueMQ2_Value)); 
    devStatusPtr->valueMQ7_Value = exchangeBytes(gizY2X(MQ7_Value_RATIO,  MQ7_Value_ADDITION, dataPoints->valueMQ7_Value)); 
    devStatusPtr->valueMQ135_Value = exchangeBytes(gizY2X(MQ135_Value_RATIO,  MQ135_Value_ADDITION, dataPoints->valueMQ135_Value)); 
